...
function makeSectorPath(x, y, angle, offsetAngle, radius) {
    if (angle >= CIRCLE) {
        return `M${x} ${y}
            m${radius} 0
    ...
    const startAngle = Math.PI / 2 - angle - offsetAngle;
    const endAngle = Math.PI / 2 - offsetAngle;
...
export default class Sector extends Component {
    ...
    render() {
        const { angle, startAngle, radius, offset, ...restProps } = this.props;
